...
import pers.tandy.chis.modules.nurseworkstation.service.ChargeFeeService;
import pers.tandy.chis.modules.nurseworkstation.service.PaymentRecordService;
import pers.tandy.chis.modules.nurseworkstation.service.RegistrationRecordService;
...
public class ChargeFeeServiceImpl implements ChargeFeeService {
    ...
    private void savePaymentRecord(User user, String lsh, Member member, PaymentRecord paymentRecord) {
        ...
        // 赋值部分属性
        float cash = paymentRecord.getCash() - paymentRecord.getCashBackAmount(); // 计算要保存的实收现金
        cash = DecimalUtils.roundHalfUp(cash, 2);

        paymentRecord.setLsh(lsh); // 流水号
        paymentRecord.setCash(cash); // 现金实收金额
    ...
    private void updateInventoryQuantity(List<SellRecord> sellRecordList) {
        ...
        for (SellRecord sellRecord : sellRecordList) {
            ...
            if (sellRecord.getSysSellTypeId().intValue() == SellTypeEnum.GOODS.getIndex().intValue()) {
                ...
                inventory.setQuantity(sellRecord.getQuantity().shortValue()); // 要修改的库存数量
